window.Janus = (function (factory) {
	...
	function Janus(gatewayCallbacks) {
		...
		if (
			typeof gatewayCallbacks.withCredentials !== 'undefined' &&
		...
		let maxev = 10
		if (
			typeof gatewayCallbacks.max_poll_events !== 'undefined' &&
		...
		let token = null
		if (
			typeof gatewayCallbacks.token !== 'undefined' &&
			gatewayCallbacks.token !== null
		)
			token = gatewayCallbacks.token
		// API secret to use (only if the shared API secret is enabled)
		let apisecret = null
		if (
			typeof gatewayCallbacks.apisecret !== 'undefined' &&
		...
		if (
			typeof gatewayCallbacks.destroyOnUnload !== 'undefined' &&
		...
		if (
			typeof gatewayCallbacks.keepAlivePeriod !== 'undefined' &&
		...
		if (
			typeof gatewayCallbacks.longPollTimeout !== 'undefined' &&
		...
		function getMaxBitrates(simulcastMaxBitrates) {
			...
			if (
				typeof simulcastMaxBitrates !== 'undefined' &&
				simulcastMaxBitrates !== null
			) {
		...
		function destroySession(callbacks) {
			...
			if (
				typeof callbacks.notifyDestroyed !== 'undefined' &&
		...
		function createHandle(callbacks) {
			...
			callbacks.connectionState =
				typeof callbacks.connectionState == 'function'
					? callbacks.connectionState
					: Janus.noop
			callbacks.iceState =
			...
			callbacks.onmessage =
				typeof callbacks.onmessage == 'function'
					? callbacks.onmessage
					: Janus.noop
			callbacks.onlocaltrack =
				typeof callbacks.onlocaltrack == 'function'
					? callbacks.onlocaltrack
					: Janus.noop
			callbacks.onremotetrack =
				typeof callbacks.onremotetrack == 'function'
					? callbacks.onremotetrack
					: Janus.noop
			callbacks.ondata =
			...
			if (websockets) {
				...
				transactions.set(transaction, function (json) {
					...
					if (json['janus'] === 'success') {
						...
						Janus.log(
							'Synchronous transaction successful (' +
								plugindata['plugin'] +
								')'
						)
						let data = plugindata['data']
		...
		function createDataChannel(
			...
			let onDataChannelStateChange = function (event) {
				...
				let dcState = config.dataChannel[label]
		...
		function createPeerconnectionIfNeeded(handleId, callbacks) {
			...
			if (callbacks.tracks) {
				for (let track of callbacks.tracks) {
					if (
						track.transforms &&
						(track.transforms.sender || track.transforms.receiver)
					) {
			...
			config.pc.ontrack = function (event) {
				...
				event.track.onended = function (ev) {
					...
					let transceiver = transceivers
						? transceivers.find((t) => t.receiver.track === ev.target)
						: null
					let mid = transceiver ? transceiver.mid : ev.target.id
					if (
						mid === ev.target.id &&
						pluginHandle.mids &&
						pluginHandle.mids[event.track.id]
					)
		...
		async function prepareWebrtc(handleId, offer, callbacks) {
			...
			if (callbacks.media && !callbacks.tracks) {
				...
				if (
					callbacks.simulcast === true ||
					callbacks.simulcast2 === true ||
					callbacks.svc
				) {
		...
		async function captureDevices(handleId, callbacks) {
			...
			for (let track of tracks) {
				...
				if (
					(typeof track.add === 'undefined' || track.add === null) &&
				...
				if (
					...
					(track.add && track.remove && track.replace)
				) {
				...
				if (track.capture) {
					if (
						track.gumGroup &&
						groups[track.gumGroup] &&
						groups[track.gumGroup].stream
					) {
						...
						if (track.type === 'audio' || track.type === 'video') {
							...
							if (track.gumGroup) {
								...
								if (
									groups[track.gumGroup] &&
									groups[track.gumGroup][otherType]
								) {
					...
					if (track.replace) {
						...
						if (track.codec) {
							if (Janus.webRTCAdapter.browserDetails.browser === 'firefox') {
								...
								let codecs = RTCRtpReceiver.getCapabilities(kind).codecs.filter(
									function (codec) {
						...
						if (track.transforms) {
							...
							if (
								...
								if (RTCRtpReceiver.prototype.createEncodedStreams) {
									...
									if (kind === 'audio') {
										receiverStreams =
											transceiver.receiver.createEncodedAudioStreams()
									} else if (kind === 'video') {
										receiverStreams =
											transceiver.receiver.createEncodedVideoStreams()
									}
								}
								if (receiverStreams) {
									if (
				...
				if (transceiver) {
					...
					if (newdir && newdir !== curdir) {
						Janus.warn(
							'Changing direction of transceiver to ' +
								newdir +
								' (was ' +
								curdir +
								')',
						...
						query.getStats().then(function (stats) {
							stats.forEach(function (res) {
								...
								if (inStats) {
									...
									if (
										config.bitrate[target].bsbefore === null ||
										config.bitrate[target].tsbefore === null
									) {
										// Skip this round
										config.bitrate[target].bsbefore =
											config.bitrate[target].bsnow
										config.bitrate[target].tsbefore =
											config.bitrate[target].tsnow
									} else {
										// Calculate bitrate
										let timePassed =
											config.bitrate[target].tsnow -
